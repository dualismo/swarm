...
def converting(name):
    name_red = "output/field/redteam/" + name
    name_sea = "output/field/seateam/" + name
    ...
    data_red_max = data_red.max()
    data_sea_max = data_sea.max()
    ...
    for i in range(cpp_height):
        for j in range(cpp_width):
            data_sea[i][j] = data_sea[i][j] / data_sea_max
            data_red[i][j] = data_red[i][j] / data_red_max
